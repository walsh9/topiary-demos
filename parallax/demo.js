...
var createTreeLine = function (treeLineOptions) {
    ...
    while (treeX < canvas.width) {
        let currentTreeOptions = treeOptions;
        currentTreeOptions.startPoint = (Topiary.Vector2d.new(treeX, canvas.height));
        currentTreeOptions.height = baseHeight * randomBetween(0.8, 1.2);
        let tree = Topiary.new(currentTreeOptions);
...
var fillWithRandomGradient = function(canvas) {
    ...
    gradient.addColorStop(0, Topiary.Color.random().toStyle());
    gradient.addColorStop(1, Topiary.Color.random().toStyle());
...
var treeColor = Topiary.Color.random();
...
var main = function () {
    ...
    currentTime = newTime;
    while (frameTime > 0) {
        let delta = Math.min(frameTime, timeStep);
        update(delta);
        frameTime -= delta;
    }
    draw();
